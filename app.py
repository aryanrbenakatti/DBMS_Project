...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://abc:abcd@localhost/aryan'
db = SQLAlchemy(app)
...
def track():
    ...
    if train:
        flash(f'Train found: {train.train_name} from {train.origin} to {train.destination}', 'success')
    else:
...
def cancel_booking():
    if request.method == 'POST':
        ...
        return redirect(url_for('index'))  # Redirect to index after cancellation
